let g:VIColorColumnBlacklist = ['diff', 'undotree', 'nerdtree', 'qf']
let g:VICursorlineBlacklist = ['command-t']
let g:VIMkviewFiletypeBlacklist = ['diff', 'hgcommit', 'gitcommit']
...
function! autocmds#should_colorcolumn() abort
  return index(g:VIColorColumnBlacklist, &filetype) == -1
endfunction

function! autocmds#should_cursorline() abort
  return index(g:VICursorlineBlacklist, &filetype) == -1
endfunction
...
function! autocmds#should_mkview() abort
  return
        \ &buftype == '' &&
        \ index(g:VIMkviewFiletypeBlacklist, &filetype) == -1 &&
